...
subtitle: 'Highly scalable technology assisted differential diagnostics of ASD'
...
bibliography: BOKI_supp-ext.bib
csl: nature.csl
header-includes:
...
# Version numbers for R and all packages
...
Throughout the manuscript, we use a threshold of α = 0.05 to assess significance. Data was preprocessed in R 4.3.0 [@R] using RStudio [@rstudio] and MATLAB R2023b [@the_mathworks_inc_matlab_2022] with all machine learning analyses computed using Neurominer V1.3 curufin [@koutsouleris_neurominer_2024]. All scripts and preprocessed, anonymised data are available on https://github.com/IreneSophia/NEVIA_ASD-BPD/tree/main/ML_BPD-ASD, while full SVM models are provided on https://osf.io/y8xbp. 
...
The Speech model mirrors the model used for classification between interaction partners in ASD-involved and COMP interactions in Plank et al. [@plank2023automated]. Similarly, five of our eight base models, namely BODYsync, FACEsync, HEADsync, INTRAsync and MovEx, as well as our stacking model mirror the models in Koehler et al. [@koehler_machine_2024]. All models were performed with the same preprocessing and classification settings as in the previous studies unless specified here. 

All models were preprocessed and analysed separately. Preprocessing for the base models included scaling the data from -1 to 1 and pruning non-informative features. Here, we slightly deviate from Koehler et al. [@koehler_machine_2024] who scaled from 0 to 1 to stay in line with Plank et al. [@plank2023automated] and the NeuroMiner recommendation. Additionally, for all base models with more than 50 features, we performed a principal component analysis, retaining the principal components explaining at least 80% of the variance, before applying pruning again. Furthermore, we used ensemble learning to choose the models performing in the top 50%, which deviates from Plank et al. [@plank2023automated] but is in line with Koehler et al. [@koehler_machine_2024].

We used a repeated, nested, stratified cross-validation structure where the inner loop allows for hyper-parameter tuning to be separate from the model evaluation in the outer loop. We implemented ten folds with ten permutations in the outer loop and nine folds with one permutation in the inner loop. Due to the dyadic nature of our data, we specifically constructed the cross-validation structure such that both interaction partners of one dyad are always in the same fold. Base models were trained using LIBLINEAR Support Vector L2-regularised L2-loss classification algorithms with balanced accuracy as the performance criterion and using hyperplane weighing for uneven group sizes. The features of the stacking model, which are the decision scores of the base models, were entered into an L1-loss LIBSVM algorithm with a Gaussian kernel and use single node learning. 
