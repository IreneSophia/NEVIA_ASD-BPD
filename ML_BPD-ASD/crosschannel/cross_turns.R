...
# set path to files
dt.pathX = c("/media/emba/emba-2/ML_BOKI/OpenFace_preprocessed_Empatica", 
            "/media/emba/emba-2/ML_BOKI/AUD_preprocessed_Empatica", 
            "/media/emba/emba-2/ML_BOKI/MEA_preprocessed_Empatica", 
...
mea.H_head = readMEA(dt.pathX[4],
                     ...
                     s1Name = "L", s2Name = "R", # differs from BOKI: left is 1-2 and right is 3-4
                     header = FALSE,
                     namefilt = "hobbies",
                     idOrder = c("x","id"),
                     idSep = "_",
...
if (length(setdiff(ls.dyads, unique(df.agg$dyad))) > 0) {
  warning("Someone is missing!")
}
