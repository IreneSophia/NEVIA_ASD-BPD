...
dt.path = c("/media/emba/emba-2/ML_BOKI/OpenFace_preprocessed_Empatica", 
...
if (!file.exists(file.path(dt.path[1], "BOKI_OF.rds"))) {
  ...
  ls.IDs = c(paste0(ls.dyads, "_H_L"), paste0(ls.dyads, "_H_R"),
             paste0(ls.dyads, "_M_L"), paste0(ls.dyads, "_M_R"))
...
df.AU.sync_NM = df.AU.sync %>%
  # translational parameters are only used in intrapersonal synchrony
...
ls.AUs.H = ls.AUs.H[!grepl("pose", ls.AUs.H)]
ls.AUs.M = ls.AUs.M[!grepl("pose", ls.AUs.M)]

# full expressiveness: mean of AUs and frames
df.exp_NM = df %>%
  select(ID, dyad, task, speaker, frame, matches("AU.*r")) %>%
  pivot_longer(names_to = "input", values_to = "exp", cols = matches("AU.*r")) %>%
  filter(
    (task == "H" & input %in% ls.AUs.H) | (task == "M" & input %in% ls.AUs.M)
  ) %>%
...
# clean workspace
rm(list = setdiff(ls(), c("df", "df.AU.sync", "df.AU.sync_NM", "df.exp_NM", 
                          "ls.fakeAU", "ls.AUs.M", "ls.AUs.H", "dt.path")))
