...
df.perf.agg = df.perf %>% group_by(model, comparison) %>% 
  summarise(ci.low = quantile(BAC, probs = 0.025, names = F), 
            ci.high = quantile(BAC, probs = 0.975, names = F) )

df.multi.agg = df.multi %>% group_by(model, comparison) %>% 
  summarise(ci.low = quantile(BAC, probs = 0.025, names = F), 
            ci.high = quantile(BAC, probs = 0.975, names = F) )
