...
dt.path = c("/media/emba/emba-2/ML_BOKI/AUD_preprocessed_Empatica", 
...
# merge the pitch limits from both samples
df.limits = rbind(
  read_csv(file.path(dt.path[1], "OUT_pitch_limits.csv")) %>% 
    select(name_short, floor_pp, ceiling_pp),
  read_csv(file.path(dt.path[1], "ML_pitch_limits.csv")) %>% 
    select(name_short, floor_pp, ceiling_pp)
  )

write_csv(df.limits, "BOKI_pitch_limits.csv")
